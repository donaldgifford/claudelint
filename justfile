# claudelint — task runner
#
# Project automation via just. Use either the Makefile or this justfile —
# both expose the same target set with equivalent behavior. Docker
# recipes live in docker.just and drive `docker buildx bake` against
# docker-bake.hcl.
...
# Build the core CLI binary into build/bin/claudelint
[group('build')]
build-core:
    @mkdir -p {{ bin_dir }}
