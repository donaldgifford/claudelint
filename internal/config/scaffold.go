...
// DefaultScaffold is the commented default .claudelint.hcl written by
// `claudelint init`. It documents every block so users have a working
// reference without needing to open the README. The file is valid HCL
// and produces zero diagnostics against a fresh repo — new users can
// run `claudelint run` immediately after init.
const DefaultScaffold = `# .claudelint.hcl — configuration for claudelint.
#
