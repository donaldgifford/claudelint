...
- **Public site** — <https://claudelint.dev>, served by Astro + Starlight
  from `site/` and deployed via Cloudflare Pages.
- **Backstage TechDocs** — built by MkDocs from `mkdocs.yml` (managed by
  `docz update`) for internal/embedded consumption.

See [DESIGN-0003](docs/design/0003-dual-output-docs-site-shared-source-mkdocs-for-techdocs.md)
for the dual-pipeline architecture. To preview the Starlight site
locally:
...
`just docs-build` runs the full Astro build; `just docs-check` runs
type + content collection diagnostics. `just lint-md` enforces
CommonMark + GFM only — no MkDocs-only syntax (`!!! note`, `???`,
`pymdownx`) so the same source renders in both pipelines.
